...
#definir chemin ou s'execute le script
#TODO: definir le chemin ##### 
#myPath=$/usr/bin
myPath=~/Documents/Variants/Pipeline_NGS_Variants/Logiciels/

############    Telecharger tabix, l'extraire, le compiler, et creation du chemin #####################

###TODO: verifier si existence repertoire??????????? faire pareil pour autres modules #####
#if [ ! -d $myPath/tabix-0.2.6 ];then
...
#TODO problem autorisation (si on met un sudo pour lancer l'execution du script ça marche non? plus besoin de mot de passe par la suite?)
...
cd $myPath/ensembl-tools-release-84/scripts/variant_effect_predictor ;  perl INSTALL.pl 

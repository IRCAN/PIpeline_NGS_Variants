	...
	def connection_ssh(self):
		...
		if self.location== "pgm":
			try:                                                            
				...
				s.prompt()
				print "Liste des runs (+++ pour runs avec CoverageAnalysis) \n" 
				s.sendline('for file in *;do if test -d $file/plugin_out/coverageAnalysis_out; then echo "+++ $file"; else echo "$file"; fi; done')
				s.readline()
